...
plot(artab$ar, cumsum(artab$n), type = 'l',xlab="År",ylab="Antal registrerade patienter",lty=c(1,1),lwd=c(3,3),col="dodgerblue2",xlim=c(2005.7,as.integer(max(artab$ar))+0.3),ylim=c(0,sum(artab$n)*1.05),yaxs="i",xaxs="i",xaxt="n")
...
# Test med medelvärde på PASI mot klinik, inte jättebra staplar ännu...
A21=as.data.frame(colMeans(as.matrix(table((Pso_contact$PASI_index),Pso_contact$Sjukhus))))
A22=as.vector(A21)
PASI=as.vector(A22$`colMeans(as.matrix(table(Pso_contact$PASI_index, Pso_contact$Sjukhus)))`)
Sjukhus=rownames(A21)
testData=data.frame(Sjukhus,PASI)
testData$n=round(PASI*100,digits = 2)



figdata <- testData %>% select(Sjukhus, PASI,n) %>% 
      mutate(quarter = cut(PASI, breaks = c(-0.01,1.5,5,8,20), labels = c('Q1','Q2','Q3','Q4'))) %>% 
      group_by(Sjukhus, quarter) %>% data.frame()


gbarplot(figdata, variable = 'quarter', sjukhus = 'Sjukhus', main = "Inskrivningar per klinik",percent = T)

gbarplot(figdata, variable = 'quarter', sjukhus = 'Sjukhus', main = "Inskrivningar per klinik",percent = T,stacked = F)
...
```{r utfallsM_arsrapport2, echo=FALSE}
...
figdata <- Pso_contact1 %>% select(Sjukhus, monad) %>% 
      mutate(quarter = cut(monad, breaks = c(0,3,6,9,12), labels = c('Q1','Q2','Q3','Q4'))) %>% 
      group_by(Sjukhus, quarter) %>%
...
Urval<- subset(Pso_contact, ContactDate > as.Date("2014-12-30") & ContactDate < as.Date("2016-01-01"))
