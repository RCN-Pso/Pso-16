...
knitr::opts_chunk$set(echo=FALSE, warning=FALSE, message=FALSE, fig.height=8, fig.width=8)
```
...
figdata <- Pso_individ_2015 %>% select(Sjukhus, Kon_value) %>% 
      mutate(Gender = cut(Kon_value, breaks = c(0,1,2), labels = c('Man','Kvinna'))) %>% 
...
```{r tabell_systembehandlingar, echo=FALSE}
...
tab_logic <- table(Pso_linjegrafdata$DrugName)/sum(table(Pso_linjegrafdata$DrugName))*100>0.5
...
tab_logic <- subset(tab_logic, logisk==TRUE)

Pso_linjegrafdata <- subset(Pso_linjegrafdata, DrugName %in% tab_logic$Systembehandling)
...
```{r area plot_systembehandlingar}
data_plot2 <-  Pso_linjegrafdata %>% group_by(År_behandling, DrugName) %>% 
        summarise(antal = n()) %>% 
        mutate(ntot = sum(antal), p = antal/ntot*100)

qplot(År_behandling, p, data=data_plot2, geom="area", fill = DrugName, xlab="År", ylab="Andel (%)") 
...
```{r utfall_ny, echo=FALSE}


Pso_individ_2015$PASI_grup <- as.numeric(cut(Pso_individ_2015$PASI_index, breaks = c(-0.1,4.9999,9.9999,500),labels=c(1,2,3)))
Pso_bio$PASI_grup <- as.numeric(cut(Pso_bio$PASI_index, breaks = c(-0.1,4.9999,9.9999,5000),labels=c(1,2,3)))

Pso_individ_2015$DLQI_grup <- as.numeric(cut(Pso_individ_2015$DLQI_index, breaks = c(-0.1,4.9999,9.9999,500),labels=c(1,2,3)))
Pso_bio$DLQI_grup <- as.numeric(cut(Pso_bio$DLQI_index, breaks = c(-0.1,4.9999,9.9999,5000),labels=c(1,2,3)))


Pso_kon=Pso_individ_2015
Pso_kon = Pso_kon[as.numeric(Pso_kon$PERSNR) %ni% as.numeric(Pso_bio$PERSNR),]
...
gbarplot(figdata, variable = 'PASI_grup', sjukhus = 'Sjukhus', main = "F",percent = T,addCI=F,sizelimit=10,plotcolors = t7_3)

figtitle = 'PASI INDEX konvent.'
...
gbarplot(figdata, variable = 'DLQI_grup', sjukhus = 'Sjukhus', main = "F",percent = T,addCI=F,sizelimit=10,plotcolors = t7_3)
...
Temp_data$BMI_kategori <- cut(Temp_data$BMI, breaks = c(0,18.49999,24.99999,29.99999,100),labels=c("Undervikt","Normalvikt", "Övervikt", "Fetma"))
...
v1=100*summary(Aktiva_behandlingar$DrugName)/length(TempData$DrugName)
...
for ( i in 1:length(v1)) {
    ...
    if (v1[i]>1){
      namn_new[i]=names(v1[i])
    }
}
namn_new=as.character(na.omit(namn_new))

TempData = subset(TempData, DrugName %in% namn_new)

TempData$Behandling_num=as.numeric(factor(TempData$DrugName))
namn=levels(factor(TempData$DrugName))
...
gbarplot(figdata, variable = 'Behandling', sjukhus = 'Sjukhus', main = "F",percent = T,addCI=F,sizelimit=10,plotcolors =mypalette)
