...
par(mfcol=c(1,1))
plot(artab$ar, cumsum(artab$n), type = 'l',xlab="År",ylab="Antal registrerade patienter",lty=c(1,1),lwd=c(3,3),col="dodgerblue2",xlim=c(2005.8,2015.3),ylim=c(0,5400),yaxs="i",xaxs="i",xaxt="n")
axis(1, at = seq(2006, 2015, by = 1), las=2)
lines(x=c(2005.8,2015.3),y=c(1000,1000),lty = 2, col ="gray") #genererar grids
lines(x=c(2005.8,2015.3),y=c(2000,2000),lty = 2, col ="gray")
lines(x=c(2005.8,2015.3),y=c(3000,3000),lty = 2, col ="gray")
lines(x=c(2005.8,2015.3),y=c(4000,4000),lty = 2, col ="gray")
lines(x=c(2005.8,2015.3),y=c(5000,5000),lty = 2, col ="gray")
...
gbarplot(figdata, variable = 'Gender', sjukhus = 'Sjukhus', main = "Inskrivningar per klinik",percent = T,addCI=F,sizelimit=2)#kolla upp vad som är man resp kvinna
...
figdata <- Pso_contact1 %>% select(Sjukhus, Smoker_Numb) %>% 
      mutate(Smoke = cut(Smoker_Numb, breaks = c(0,1,2,3), labels = c('Ja','Slutat','Nej'))) %>% 
      group_by(Sjukhus, Smoke) %>%
      summarise(n = n()) %>% data.frame()

gbarplot(figdata, variable = 'Smoke', sjukhus = 'Sjukhus', main = "Andel rökare",percent = T,addCI=F,sizelimit=2)#kolla upp vad som är man resp kvinna
```
