...
Pso_linjegrafdata$DrugName2 <- as.character(Pso_linjegrafdata$DrugName)



for(i in 1:nrow(tab_logic)){
Pso_linjegrafdata$DrugName2[Pso_linjegrafdata$DrugName2==as.character(tab_logic$Systembehandling)[i]] <- "övriga systembehandlingar"
}

Pso_linjegrafdata$DrugName2 <- factor(Pso_linjegrafdata$DrugName2)
Pso_linjegrafdata$DrugName2 <-factor(Pso_linjegrafdata$DrugName2,levels(Pso_linjegrafdata$DrugName2)[c(5,1,3,2,4,6,7)])
...
data_plot2 <-  Pso_linjegrafdata %>% group_by(År_behandling, DrugName2) %>% 
        summarise(antal = n()) %>% 
        mutate(ntot = sum(antal), p = antal/ntot*100)

fargval<-brewer.pal(length(levels(data_plot2$DrugName2)),"Set3")
fargval <- rep(fargval,5)

qplot(År_behandling, p, data=data_plot2, geom="area", fill = DrugName2, xlab="År", ylab="Andel (%)") 
...
gbarplot(figdata, variable = 'Behandling', sjukhus = 'Sjukhus', main = "",percent = T,addCI=F,sizelimit=10,plotcolors =mypalette)
...
gbarplot(figdata, variable = 'Smoke', sjukhus = 'Sjukhus', main = "Rökning",percent = T,addCI=F,sizelimit=10,plotcolors = t7_3) #kolla upp vad som är man resp kvinna
